...
def analisar_transacao(lista_transacoes):
    ...
    lista_mensagens = [
        ...
        {
                "role": "user",
                "content": f"Considere o CSV abaixo, onde cada linha é uma transação diferente: {lista_transacoes}. Sua resposta deve adotar o # Formato Saída (apenas um json sem outros comentários)"
        }
    ]
...
def gerar_recomendacao(parecer):
    ...
    da transação da Transação: {parecer}

    As recomendações podem ser "Notificar Cliente", "Acionar setor Anti-Fraude" ou 
    "Realizar Verificação Manual". Elas devem ser escritas no formato técnico.
...
for uma_transacao in transacoes_analisadas["transacoes"]:
    if uma_transacao["status"] == "Possível Fraude":
        ...
        produto_trasancao = uma_transacao["nome_produto"]
        status_transacao = uma_transacao["status"]
        
        salva(f"transacao-{id_transacao}-{produto_trasancao}-{status_transacao}.txt", recomendacao)